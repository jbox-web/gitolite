describe Gitolite::Config do
  ...
  describe "Subconfig managment" do
    before :each do
      @root_config = Gitolite::Config.load_from(File.join(conf_dir, 'complicated.conf'))
    ...
    describe "#parent=" do
      it 'should raise a ConfigDependencyError if there is a cyclic dependency' do
        ...
        expect{s.add_subconf c}.to raise_error(Gitolite::Config::ConfigDependencyError)
      end
    end
    ...
    describe "#to_file" do
      it 'should ensure save subconfs info' do
        ...
        lines[2].should == "include    \"subconf1.conf\""
        # Cleanup
        File.unlink(file)
      ...
      it 'should ensure save subconfs info and force to create the directory ' do
        c = Gitolite::Config.init
        c.filename = "test_subconfs.conf"
        ...
        lines = f.lines.map {|l| l.strip}
        # Compare the file lines.  Spacing is important here since we are doing a direct comparision
        lines[2].should == "include    \"mytest_subconf/subconf1.conf\""
        # Cleanup
        File.unlink(file)
